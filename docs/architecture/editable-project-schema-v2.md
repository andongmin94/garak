...
- 상태: Phase 2A current editable source contract; Windows x64 validation PASS
- Current schema version: `2`
- Legacy supported input: schema version `1`
...
Schema v2는 Garak의 current editable `.garak` project source contract다. v1의 product identity,
white-label metadata와 Gain product 의미를 그대로 보존하면서 template identity와 template contract
version을 별도 field로 표현한다.
...
이 문서는 current v2 logical/JSON contract를 정의한다. Legacy v1 exact shape는
...
Legacy v1 source는 v1 validator와 pure migration을 거쳐 같은 canonical v2 model에 도달한다. Compile과
export는 `string | object` 같은 legacy union을 받지 않는다. Canonical `ProductProject`와 versioned source
...
Current writer는 다음 exact 규칙으로 v2를 serialize한다.
...
동일한 validated logical product는 JSON source의 original whitespace/key order와 실행 환경에 관계없이
동일한 canonical v2 bytes와 SHA-256을 만든다. Canonical serialization은 source open의 부수 효과가
아니며 explicit create/save 또는 migration output publication에서만 filesystem에 기록된다.
...
같은 v1/v2 logical product는 exact same `GARAKCPD` v1 bytes, Product ID/FUID, parameter table,
...
Schema v2는 current editorless Gain product의 최소 evolution이다. Graph, arbitrary node, macro, interface,
...
Product Runtime, preset/DAW/plugin state의 compatibility policy는 Phase 2C다. Future project schema는
명시적 ADR, exact validator, sequential migration step과 fixture 없이 current v2로 읽지 않는다.
